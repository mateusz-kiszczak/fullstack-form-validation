...
// Age.
function is_number($number, $min = 0, $max = 999): bool {
...
function is_phone_number($phone, $min = 9, $max = 13) {
  ...
  if ( $isLength and preg_match('/[0-9]/', $phone) ) {
    return true;
  }
  return false;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $errors['name'] = is_text($userInput['name'], 2, 64) ? '' : 'Name must be 2-64 letters.';
  ...
  $errors['password'] = is_password_strong($userInput['password']) ? '' : 'Your password is not strong enough. Password needs at least one big letter, one number and one of the following special characters: ! £ $ & ? @ . : ; _ -.';
